...
# Join with outcome data and SPLIT column
...
print(f"Total rows (full cohort): {total_rows:,}")
print(f"Training rows (for feature selection): {total_train_rows:,}")
print(f"Baseline CRC rate (train only): {baseline_crc_rate:.4f}")

feature_cols = [c for c in df_spark.columns if c not in ['PAT_ID', 'END_DTTM', 'FUTURE_CRC_EVENT', 'SPLIT']]
...
# CELL 17 - Step 2: Calculate Risk Ratios for Binary Features (TRAIN DATA ONLY)
print("\nCalculating risk ratios for binary features (using training data only)...")
...
for feat in binary_features:
    if 'CACHEXIA_RISK_SCORE' in feat:
        ...
        stats = df_train.filter(F.col(feat) == 2).agg(
            F.count('*').alias('count'),
            F.avg('FUTURE_CRC_EVENT').alias('crc_rate')
        ).collect()[0]

        prevalence = stats['count'] / total_train_rows
...
# CELL 18 - Step 3: Calculate Mutual Information on Stratified Sample (TRAIN DATA ONLY)
print("\nCalculating Mutual Information (using training data only)...")
...
print(f"Sampled {len(df_sample):,} rows from TRAINING data for MI calculation")
