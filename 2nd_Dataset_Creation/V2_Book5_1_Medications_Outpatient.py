...
# Step 2: Calculate Risk Ratios for Flag Features (TRAIN DATA ONLY)
...
# Step 4: Sample for Mutual Information (TRAIN DATA ONLY)
...
print(f"\nSampled {len(df_sample):,} rows from TRAINING data for MI calculation ({len(df_sample)/total_train_rows*100:.1f}%)")
print(f"Sample CRC rate: {df_sample['FUTURE_CRC_EVENT'].mean():.4f}")

# Calculate MI on sample
from sklearn.feature_selection import mutual_info_classif

feature_cols = [c for c in df_sample.columns if c not in ['PAT_ID', 'END_DTTM', 'FUTURE_CRC_EVENT', 'SPLIT']]
