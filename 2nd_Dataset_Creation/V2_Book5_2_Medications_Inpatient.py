...
df_spark = spark.sql("""
    SELECT m.*, c.FUTURE_CRC_EVENT, c.SPLIT
...
print([col for col in df_spark.columns if col not in ['PAT_ID', 'END_DTTM', 'FUTURE_CRC_EVENT', 'SPLIT']][:5])
...
# Step 2: Calculate Risk Ratios for Inpatient Flag Features (TRAIN DATA ONLY)

flag_features = [col for col in df_spark.columns if '_flag' in col and col.startswith('inp_')]
risk_metrics = []

print(f"\nCalculating risk ratios for {len(flag_features)} flag features (using training data only)...")
...
# Step 4: Sample for Mutual Information Calculation (TRAIN DATA ONLY)
...
sample_fraction = min(200000 / total_train_rows, 1.0)  # Larger sample for inpatient
